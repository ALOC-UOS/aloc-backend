...
public class UserProblemService {
  ...
  public boolean isProblemAlreadySolved(Long userId, Long problemId) {
    ...
    return userProblemRepository.existsByUserIdAndProblemIdAndUserProblemStatus(
        userId, problemId, UserProblemStatus.SOLVED);
  }
  ...
  public UserProblem getOrCreateUserProblem(
      User user, Problem problem, UserProblemStatus userProblemStatus) {
  ...
  public List<UserProblem> getSolvedUserListByProblemId(Long problemId) {
    return userProblemRepository.findAllByProblemIdAndUserProblemStatus(
        problemId, UserProblemStatus.SOLVED);
  }

  // 시즌, 풀이 여부, 루틴에 따라 유저의 문제 목록을 가져옵니다.
  public List<UserProblem> getUserProblemList(
      Long userId, Integer season, UserProblemStatus userProblemStatus) {
    System.out.println(
        userProblemRepository.findAllByUserIdAndSeasonAndIsSolvedOrderBySolvedAtDesc(
            userId, season, userProblemStatus));
  ...
  public int getSolvedUserCount(Long problemId) {
    return userProblemRepository.countSolvingUsersByProblemId(

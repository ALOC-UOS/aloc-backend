...
class AlgorithmServiceTest {
  ...
  void getAlgorithms() {
    // given
    when(algorithmRepository.findAllByHiddenIsFalseOrderByWeekDesc()).thenReturn(algorithms);
    ...
    AlgorithmResponseDto season1 =
        result.stream().filter(r -> r.getSeason() == 1).findFirst().orElse(null);
    ...
    AlgorithmResponseDto season2 =
        result.stream().filter(r -> r.getSeason() == 2).findFirst().orElse(null);

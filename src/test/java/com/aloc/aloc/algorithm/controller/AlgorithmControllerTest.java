...
class AlgorithmControllerTest {
  ...
  private AlgorithmResponseDto createAlgorithmResponseDto(int season) {
    List<AlgorithmDto> algorithmDtos =
        Arrays.asList(
  ...
  void testGetAlgorithms() throws Exception {
    ...
    mockMvc
        .perform(get("/api2/algorithm").contentType(MediaType.APPLICATION_JSON))
  ...
  void testGetAlgorithmsBySeason() throws Exception {
    ...
    mockMvc
        .perform(get("/api2/algorithm/{season}", 1).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.isSuccess").value(true))
        .andExpect(jsonPath("$.code").value("COMMON200"))
        .andExpect(jsonPath("$.message").value("성공입니다."))

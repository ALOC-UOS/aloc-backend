...
public class ProblemControllerTest {
  ...
  void getSolvedUserListShouldReturnListOfSolvedUsers() throws Exception {
    // Given
    Long problemId = 1L;
    List<SolvedUserResponseDto> solvedUsers =
        Arrays.asList(
    ...
    mockMvc
        .perform(
            get("/api2/problem/{problemId}/solved-users", problemId)
  ...
  void getTodayProblemWithFullCourseShouldReturnTodayProblem() throws Exception {
    // Given
    when(problemService.getTodayProblemDto(Course.FULL))
        .thenReturn(
    ...
    mockMvc
        .perform(
            get("/api2/today-problem?course={course}", Course.FULL)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.isSuccess").value(true))
        .andExpect(jsonPath("$.code").value("COMMON200"))
  ...
  void getTodayProblemWithHalfCourseShouldReturnTodayProblem() throws Exception {
    // Given
    when(problemService.getTodayProblemDto(Course.HALF))
        .thenReturn(
            ProblemResponseDto.builder()
                .id(1L)
                .problemId(1)
    ...
    mockMvc
        .perform(
            get("/api2/today-problem?course={course}", Course.HALF)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.isSuccess").value(true))
        .andExpect(jsonPath("$.code").value("COMMON200"))
  ...
  void getTodayProblemWithFullCourseNotFoundShouldReturnException() throws Exception {
    // Given
    ...
    mockMvc
        .perform(
            get("/api2/today-problem?course={course}", course)
                .contentType(MediaType.APPLICATION_JSON))
  ...
  void getTodayProblemWithHalfCourseNotFoundShouldReturnException() throws Exception {
    // Given
  ...
  void getTodayProblemWithInvalidCourseShouldReturnBadRequest() throws Exception {
    // Given
  ...
  void getProblemsByAlgorithmIdAndSeason() throws Exception {
    ...
    List<ProblemResponseDto> problems =
        Arrays.asList(
            new ProblemResponseDto(1L, 1, "Problem 1", null, 3, 100),
            new ProblemResponseDto(2L, 2, "Problem 2", null, 4, 50));
    when(problemService.getVisibleDailyProblemsDtoByAlgorithmId(season, algorithmId, Course.FULL))
        .thenReturn(problems);
    ...
    mockMvc
        .perform(
            get(
                    "/api2/problems?season={season}&algorithmId={algorithmId}&course=FULL",
                    season,
                    algorithmId)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
